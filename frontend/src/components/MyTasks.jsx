...
function MyTasks() {
  ...
  const [isFullAccess, setIsFullAccess] = useState(false); // Глобальные админы/директора
  ...
  const fetchData = async () => {
    try {
      const [tasksRes, projectsRes, usersRes] = await Promise.all([
        api.get('tasks/?assigned_to_me=true'), // Сервер сам отфильтрует и скроет hidden_for
        api.get('projects/'),
        api.get('users/').catch(() => ({ data: [] }))
      ]);

      // Хронологическая сортировка для правильного порядка
  ...
  const handleDragEnd = async (result) => {
    ...
    // Скрытие задачи для участников (в "Мои задачи" участники обычно не попадают, но на всякий случай)
  ...
  // --- ПРАВА ДЛЯ МОДАЛКИ РЕДАКТИРОВАНИЯ ---
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              ...
              <div className={`w-full ${canEditAll ? 'md:w-3/5' : 'md:w-1/2'} p-6 md:p-8 overflow-y-auto flex flex-col border-b md:border-b-0 md:border-r border-gray-200 bg-white pb-24 md:pb-8`}>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  {canEditAll && <button onClick={() => handleQuickDelete(editingTask.id)} className="text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg text-sm transition-colors whitespace-nowrap ml-3 shrink-0">Удалить</button>}
                </div>
